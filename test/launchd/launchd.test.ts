...
test("verify is read-only and install lifecycle is explicit and disposable", async () => {
  ...
  await mkdir(workdir);
  const env = { ...process.env, HOME: home, AO_LAUNCHD_HOME: home, AO_LAUNCHD_LAUNCHCTL: fakeLaunchctl, AO_LAUNCHD_NODE: process.execPath, AO_LAUNCHD_CLI: cli, AO_LAUNCHD_WORKDIR: workdir, AO_LAUNCHD_LOG_DIR: join(home, "logs"), AO_CONFIG_PATH: config };
  ...
  assert.match(rendered, new RegExp(`<string>${escapeRegExp(config)}<\\/string>`));
