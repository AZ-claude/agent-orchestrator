...
この不変条件を起動時に取りこぼさないため、canonical manifest は `workerCompletionContract` と `implementationPromptTemplate` を持つ。dispatcher は [Luna implementation-task template](../prompts/luna-implementation-task.md) とこの契約を task 固有 prompt に必ず注入する。implementation Luna は `independentReview: required` を返すだけで終わってはならず、同じ session 内で reviewer subagent を起動し、`APPROVE` または capability failure を structured result として返す。後者のみが別 Luna session fallback を許可する根拠となる。
