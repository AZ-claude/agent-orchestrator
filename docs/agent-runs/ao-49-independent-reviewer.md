...
review starts a fresh ephemeral `codex exec` process in the assigned worktree
with `--sandbox read-only`; no implementation session ID, prompt history, or
rework conversation crosses the boundary. The prompt contains only the
...
`CAPABILITY_UNAVAILABLE`, so the controller cannot close an Issue on reviewer
capability failure.
