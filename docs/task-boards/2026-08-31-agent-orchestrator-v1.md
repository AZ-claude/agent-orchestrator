...
reviewer subagent の標準 prompt は implementation Luna が structured completion result を出した後に自動起動するため、各 Task や Terra に review 用の別指示を追加しない。subagent 非対応環境だけ同じ prompt を別 Luna session に渡す。
