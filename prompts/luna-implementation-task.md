# Primary/Recovery Worker implementation-task prompt template
...
verification commands from the canonical task manifest (for this delta:
`tasks/agent-orchestrator-preinstall-delta.yaml`).

```text
You are the implementation Worker for exactly one task. Read the handoff,
...
Independent-review completion contract (mandatory; Terra is not a normal
task reviewer or merge operator):
...
5. Do not report the task complete or request Terra's normal review. Report
   `independentReview: approved`, reviewer evidence, and the reviewed HEAD
   only after APPROVE. A Recovery Worker receives durable evidence only and
   starts a fresh session; it never resumes the retired Primary session.
